...
import { fetchHtml } from "../services/requests.js";
...
export class X1337Provider implements Provider {
  ...
  async search(
    ...
    return dom.findAll("tbody > tr").map((el) => {
      const id = (el.findAll("a")[1]?.attrs.href || "").split("/")[2] || "";
      return {
        id,
        ...
        date:
          this.parseDate(el.find(".coll-date")?.text?.trim() ?? "").getTime() ||
          undefined,
