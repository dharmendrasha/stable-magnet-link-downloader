...
export class TorrentEnum1705825071234 implements MigrationInterface {
  name = "TorrentEnum1705825071234";

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "magnet_requests" ADD "message" character varying`,
    );
    await queryRunner.query(
      `ALTER TYPE "public"."magnet_requests_status_enum" RENAME TO "magnet_requests_status_enum_old"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."magnet_requests_status_enum" AS ENUM('in_progress', 'paused', 'failed', 'completed', 'noted', 'in_queue')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."magnet_requests_status_enum_old" AS ENUM('in_progress', 'paused', 'failed', 'completed', 'noted')`,
    );
    await queryRunner.query(
      `ALTER TABLE "magnet_requests" ALTER COLUMN "status" DROP DEFAULT`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "magnet_requests" DROP COLUMN "message"`,
    );
  }
}
