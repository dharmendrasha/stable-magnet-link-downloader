...
export class TorrentFileDefStatus1705161135241 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."magnet_requests_status_enum" AS ENUM('in_progress', 'paused', 'failed', 'completed', 'noted')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."magnet_requests_status_enum_old" AS ENUM('in_progress', 'paused', 'failed', 'completed')`,
    );
    await queryRunner.query(
      `ALTER TABLE "magnet_requests" ALTER COLUMN "status" DROP DEFAULT`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "magnet_requests" ALTER COLUMN "status" SET DEFAULT 'in_progress'`,
    );
